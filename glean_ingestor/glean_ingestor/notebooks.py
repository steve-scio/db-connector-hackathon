...
from .constants import send_request, DATASOURCE_NAME, NOTEBOOK_OBJECT_NAME, BASE_URL, API_CLIENT
from typing import List
import logging
import os

def crawl_notebooks(upload_id: str):
    remainingDirs = ['/']
    while(remainingDirs):
        notebooks = [] 
        ...
        for asset in (response and response["objects"]):
            asset_type = asset["object_type"]
            if(asset_type == "DIRECTORY"):
                remainingDirs.append(asset['path'])
...
def upload_notebooks(notebooks: List[dict], upload_id: str):
    ...
    if(notebooks):
        ...
        for notebook in notebooks:
            documents.append(
                DocumentDefinition(
                    ...
                    object_type=NOTEBOOK_OBJECT_NAME,
        ...
        logging.info("Bulk indexed %d notebooks" %
            (len(documents)))